...
import { convertMarkdownToBbcode } from '@/lib/markdownToBbcode';

const SAMPLE = "# Announcements\n**Welcome** to our forum! Check [Docs](https://devstools.app).";

export default function MarkdownToBbcodeTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';
  let error = '';

  try {
    output = convertMarkdownToBbcode(input);
  } catch (e: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Markdown Document</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Forum BBCode Markup</label>
