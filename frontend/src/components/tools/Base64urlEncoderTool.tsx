...
import { encodeBase64Url } from '@/lib/base64urlEncoder';

export default function Base64urlEncoderTool() {
  const [input, setInput] = useState("Hello World URL-Safe Payload?");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof encodeBase64Url === 'function' ? String(encodeBase64Url(input)) : '';
    } catch (e: any) {
