...
import { curlToCsharp } from '@/lib/curlToCsharp';
import { Copy, Check } from 'lucide-react';

export default function CurlToCsharpTool() {
  const [curl, setCurl] = useState('curl https://api.example.com/items');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        />
        <pre className="h-[235px] overflow-auto rounded-2xl border border-slate-200 bg-slate-900 p-4 font-mono text-xs text-blue-400 dark:border-white/10">
          {code}
        </pre>
      </div>
    </div>
