...
import { convertBbcodeToMarkdown } from '@/lib/bbcodeToMarkdown';

const SAMPLE = "[size=6][b]Announcements[/b][/size]\n[b]Welcome[/b] to our forum! Check [url=https://devstools.app]Docs[/url].";

export default function BbcodeToMarkdownTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';
  let error = '';

  try {
    output = convertBbcodeToMarkdown(input);
  } catch (e: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">BBCode Formatting</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">GitHub Markdown</label>
