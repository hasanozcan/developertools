...
import { evaluateRegexRisk } from '@/lib/regexBenchmarkSimulator';

const SAMPLE = "^(a+)+$";

export default function RegexBenchmarkSimulatorTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = evaluateRegexRisk(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Regular Expression Pattern</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">ReDoS Backtracking Security Analysis</label>
