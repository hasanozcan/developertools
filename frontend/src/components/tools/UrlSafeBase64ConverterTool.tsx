...
import { toUrlSafeBase64 } from '@/lib/urlSafeBase64Converter';

const SAMPLE = "a+b/c==";

export default function UrlSafeBase64ConverterTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = toUrlSafeBase64(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Standard Base64 String</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">URL & Filename Safe Base64</label>
