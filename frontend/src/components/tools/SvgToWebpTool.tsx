...
import { getSvgDataUri } from '@/lib/svgToWebp';

export default function SvgToWebpTool() {
  const [input, setInput] = useState("<svg width=\"100\" height=\"100\"><circle cx=\"50\" cy=\"50\" r=\"40\" fill=\"purple\"/></svg>");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof getSvgDataUri === 'function' ? String(getSvgDataUri(input)) : '';
    } catch (e: any) {
