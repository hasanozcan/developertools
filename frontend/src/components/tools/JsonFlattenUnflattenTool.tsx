...
import { flattenJson } from '@/lib/jsonFlattenUnflatten';

export default function JsonFlattenUnflattenTool() {
  const [input, setInput] = useState("sample input");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof flattenJson === 'function' ? String(flattenJson(input)) : '';
    } catch (e: any) {
