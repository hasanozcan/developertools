...
import { formatDockerCompose } from '@/lib/dockerComposeFormatter';

export default function DockerComposeFormatterTool() {
  const [input, setInput] = useState("version: \"3.8\"\\nservices:\\n  web:\\n    image: nginx");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof formatDockerCompose === 'function' ? String(formatDockerCompose(input)) : '';
    } catch (e: any) {
