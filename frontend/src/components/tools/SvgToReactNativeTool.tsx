...
import { convertSvgToReactNative } from '@/lib/svgToReactNative';

export default function SvgToReactNativeTool() {
  const [input, setInput] = useState("<svg width=\"24\" height=\"24\"><path d=\"M12 2L2 22h20L12 2z\"/></svg>");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertSvgToReactNative === 'function' ? String(convertSvgToReactNative(input)) : '';
    } catch (e: any) {
