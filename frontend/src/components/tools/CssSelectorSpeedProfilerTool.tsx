...
import { calculateSpecificity } from '@/lib/cssSelectorSpeedProfiler';

const SAMPLE = "#header .nav-item a.active";

export default function CssSelectorSpeedProfilerTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = calculateSpecificity(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">CSS Selector String</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Specificity Score [ID, Class, Element]</label>
