...
import { httpHeadersToJson } from '@/lib/httpHeadersToJson';
import { Copy, Check } from 'lucide-react';

export default function HttpHeadersToJsonTool() {
  const [headers, setHeaders] = useState('Content-Type: application/json\nAuthorization: Bearer my_jwt_token\nCache-Control: no-cache');
  const [copied, setCopied] = useState(false);
  const json = httpHeadersToJson(headers);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        />
        <pre className="h-[200px] overflow-auto rounded-2xl border border-slate-200 bg-slate-900 p-4 font-mono text-xs text-teal-400 dark:border-white/10">
          {json}
        </pre>
      </div>
    </div>
