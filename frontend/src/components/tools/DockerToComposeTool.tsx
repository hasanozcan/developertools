...
import { convertDockerRunToCompose } from '@/lib/dockerToCompose';

export default function DockerToComposeTool() {
  const [input, setInput] = useState("docker run -d -p 3000:3000 -e PORT=3000 node:20");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertDockerRunToCompose === 'function' ? String(convertDockerRunToCompose(input)) : '';
    } catch (e: any) {
