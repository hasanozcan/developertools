...
import { generateFetchClientFromOpenApi } from '@/lib/openapiToTypescriptFetch';

export default function OpenapiToTypescriptFetchTool() {
  const [input, setInput] = useState("openapi: 3.0.0\\ninfo:\\n  title: Sample API\\n  version: 1.0.0");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof generateFetchClientFromOpenApi === 'function' ? String(generateFetchClientFromOpenApi(input)) : '';
    } catch (e: any) {
