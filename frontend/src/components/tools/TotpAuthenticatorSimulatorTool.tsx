...
import { computeSimulatedTotp } from '@/lib/totpAuthenticatorSimulator';

export default function TotpAuthenticatorSimulatorTool() {
  const [input, setInput] = useState("sample input");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof computeSimulatedTotp === 'function' ? String(computeSimulatedTotp(input)) : '';
    } catch (e: any) {
