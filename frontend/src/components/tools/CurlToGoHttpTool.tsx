...
import { convertCurlToGoHttp } from '@/lib/curlToGoHttp';

const SAMPLE = "curl https://api.coingecko.com/api/v3/ping";

export default function CurlToGoHttpTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';
  let error = '';

  try {
    output = convertCurlToGoHttp(input);
  } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Go net/http Request Code</label>
