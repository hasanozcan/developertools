...
import { encodeMorseCode } from '@/lib/morseCodeAudioConverter';

export default function MorseCodeAudioConverterTool() {
  const [input, setInput] = useState("SOS DEVELOPER TOOLS");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof encodeMorseCode === 'function' ? String(encodeMorseCode(input)) : '';
    } catch (e: any) {
