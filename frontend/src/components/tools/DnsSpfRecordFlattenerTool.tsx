...
import { countSpfLookups } from '@/lib/dnsSpfRecordFlattener';

const SAMPLE = "v=spf1 include:_spf.google.com include:sendgrid.net ~all";

export default function DnsSpfRecordFlattenerTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = countSpfLookups(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Raw SPF TXT Record</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">DNS Lookups Count (Max 10 Limit)</label>
