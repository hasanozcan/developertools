...
import { inspectGlyph } from '@/lib/unicodeGlyphCategoryInspector';

const SAMPLE = "🚀";

export default function UnicodeGlyphCategoryInspectorTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = inspectGlyph(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Unicode Character / Emoji</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Code Point & Hex Value</label>
