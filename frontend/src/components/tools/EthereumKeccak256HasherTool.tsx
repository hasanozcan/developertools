...
import { computeKeccakSimulation } from '@/lib/ethereumKeccak256Hasher';

export default function EthereumKeccak256HasherTool() {
  const [input, setInput] = useState("transfer(address,uint256)");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof computeKeccakSimulation === 'function' ? String(computeKeccakSimulation(input)) : '';
    } catch (e: any) {
