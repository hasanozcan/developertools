...
import { inspectOgTags } from '@/lib/opengraphTagInspector';

const SAMPLE = "<meta property=\"og:title\" content=\"DevsTools\" />\n<meta property=\"og:description\" content=\"Online developer tools\" />";

export default function OpengraphTagInspectorTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';

  try {
    const res = inspectOgTags(input);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">HTML Meta Tags</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Extracted OpenGraph Attributes</label>
