...
import { convertZodCodeToJsonSchema } from '@/lib/zodToJsonSchema';

export default function ZodToJsonSchemaTool() {
  const [input, setInput] = useState("const User = z.object({\\n  id: z.number(),\\n  name: z.string()\\n});");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertZodCodeToJsonSchema === 'function' ? String(convertZodCodeToJsonSchema(input)) : '';
    } catch (e: any) {
