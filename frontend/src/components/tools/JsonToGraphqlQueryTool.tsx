...
import { convertJsonToGraphqlQuery } from '@/lib/jsonToGraphqlQuery';

export default function JsonToGraphqlQueryTool() {
  const [input, setInput] = useState("{\\n  \"user\": {\\n    \"id\": 1,\\n    \"name\": \"Alice\"\\n  }\\n}");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertJsonToGraphqlQuery === 'function' ? String(convertJsonToGraphqlQuery(input)) : '';
    } catch (e: any) {
