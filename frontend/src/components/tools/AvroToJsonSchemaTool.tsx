...
import { convertAvroToJsonSchema } from '@/lib/avroToJsonSchema';

export default function AvroToJsonSchemaTool() {
  const [input, setInput] = useState("{\\n  \"type\": \"record\",\\n  \"name\": \"User\",\\n  \"fields\": [{ \"name\": \"id\", \"type\": \"long\" }]\\n}");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertAvroToJsonSchema === 'function' ? String(convertAvroToJsonSchema(input)) : '';
    } catch (e: any) {
