...
import { validateSolanaAddress } from '@/lib/solanaAddressValidator';

export default function SolanaAddressValidatorTool() {
  const [input, setInput] = useState("7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof validateSolanaAddress === 'function' ? String(validateSolanaAddress(input)) : '';
    } catch (e: any) {
