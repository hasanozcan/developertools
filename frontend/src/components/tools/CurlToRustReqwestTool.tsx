...
import { convertCurlToRustReqwest } from '@/lib/curlToRustReqwest';

const SAMPLE = "curl -X POST https://api.anthropic.com/v1/messages \\\n  -H \"Content-Type: application/json\"";

export default function CurlToRustReqwestTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';
  let error = '';

  try {
    output = convertCurlToRustReqwest(input);
  } catch (e: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">cURL Command</label>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Rust reqwest Async Client</label>
