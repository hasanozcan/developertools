...
import { computeNextCronRuns } from '@/lib/cronNextRunsVisualizer';

export default function CronNextRunsVisualizerTool() {
  const [input, setInput] = useState("0 */2 * * *");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof computeNextCronRuns === 'function' ? String(computeNextCronRuns(input)) : '';
    } catch (e: any) {
