...
import { convertSrtToWebVtt } from '@/lib/subtitleSrtVttConverter';

export default function SubtitleSrtVttConverterTool() {
  const [input, setInput] = useState("1\\n00:00:01,000 --> 00:00:04,000\\nWelcome to DevsTools");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof convertSrtToWebVtt === 'function' ? String(convertSrtToWebVtt(input)) : '';
    } catch (e: any) {
