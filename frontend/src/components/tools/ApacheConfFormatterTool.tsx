...
import { formatApacheConf } from '@/lib/apacheConfFormatter';

export default function ApacheConfFormatterTool() {
  const [input, setInput] = useState("<VirtualHost *:80>\\nServerName test.com\\n</VirtualHost>");
  const [copied, setCopied] = useState(false);

  const result = useMemo(() => {
    try {
      return typeof formatApacheConf === 'function' ? String(formatApacheConf(input)) : '';
    } catch (e: any) {
