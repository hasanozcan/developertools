...
import { convertSqlToGoGorm } from '@/lib/sqlToGoGorm';

const SAMPLE = "CREATE TABLE users (\n  id INT PRIMARY KEY,\n  email VARCHAR(255) NOT NULL,\n  created_at TIMESTAMP\n);";

export default function SqlToGoGormTool() {
  const [input, setInput] = useState(SAMPLE);
  let output = '';
  let error = '';

  try {
    output = convertSqlToGoGorm(input);
  } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Go GORM Model Structs</label>
